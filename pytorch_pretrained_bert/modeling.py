...
#my config
vis_embedding_dim = myconfig.vis_embedding_dim
vis_input = myconfig.vis_input
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.vis_pe_embeddings = nn.Embedding(
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids=None, position_ids=None, vis_input=myconfig.vis_input, len_vis_input=49):
        ...
        if vis_input:
            vis_pe = torch.arange(
                len_vis_input, dtype=torch.long, device=vis_feats.device)
            vis_pe = vis_pe.unsqueeze(0).expand((input_ids.shape[0], vis_pe.shape[0]))
            vis_pe = self.vis_pe_embeddings(vis_pe)
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, enable_butd=False, len_vis_input=49, tasks='img2txt'):
        ...
        if enable_butd:
            self.vis_embed = nn.Sequential(nn.Linear(myconfig.vis_embedding_dim, myconfig.vis_embedding_dim),
                                       nn.ReLU(),
                                       nn.Linear(myconfig.vis_embedding_dim, config.hidden_size),
                                       nn.ReLU(),
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, ans_labels=None, next_sentence_label=None, masked_pos=None, masked_weights=None, task_idx=None, vis_masked_pos=[], mask_image_regions=False, drop_worst_ratio=0.2, vqa_inference=False):
        ...
        if self.tasks == 'vqa2':
            ...
            return masked_lm_loss.new(1).fill_(0), vqa2_pred, vqa2_loss # works better when combined with max_pred=1
        else:
            if myconfig.pretrain_mode == 'test' or myconfig.pretrain_mode == 'test_td':
                return masked_lm_loss, torch.argmax(prediction_scores_masked, dim=2), masked_lm_loss.new(1).fill_(0)
            elif myconfig.pretrain_mode == 'train':
