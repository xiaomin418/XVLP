...
class MyTokenizer(object):
    def __init__(self, vocab_path, do_lower_case=True, max_len=None, never_split=("[UNK]", "[SEP]", "[X_SEP]", "[PAD]", "[CLS]", "[MASK]")):
        lines = open(vocab_path, 'r').readlines()
        specical_tokens = ["[UNK]", "[SEP]", "[X_SEP]", "[PAD]", "[CLS]", "[MASK]"]
        add_tokens = []
        lines = [line.strip() for line in lines]
        for sp in specical_tokens:
            if sp not in lines:
                add_tokens.append(sp)
        lines = add_tokens + lines
        lines = lines[:myconfig.vocab_size]
        vocab = collections.OrderedDict()
    ...
    def tokenize(self, text):
        split_words = [token.text.lower() for token in spacy_eng.tokenizer(text)]
        return split_words

    def convert_tokens_to_ids(self, tokens):
        """Converts a sequence of tokens into ids using the vocab."""
        # print("convert_tokens_to_ids:",tokens)
        ids = []
        for token in tokens:
            if token not in self.vocab:
