...
from pytorch_pretrained_bert.tokenization import BertTokenizer, WhitespaceTokenizer, MyTokenizer
...
def main():
    ...
    parser.add_argument('--region_bbox_file', default='./data/flickr30k/region_feat_gvd_wo_bgd/flickr30k_detection_vg_thresh0.2_feat_gvd_checkpoint_trainvaltest.h5', type=str)
    ...
    if myconfig.tokenizer_name == 'mytokenizer':
        tokenizer = MyTokenizer('./data/vocab_itos.txt', do_lower_case=args.do_lower_case)
    elif myconfig.tokenizer_name == 'bertokenizer':
    ...
    if args.do_train:
        ...
        for i_epoch in trange(0, 1, desc="Epoch"):
            ...
            for step, batch in enumerate(iter_bar):
                if myconfig.pretrain_mode == 'test' and myconfig.dev_data_gen:
                    dev_npy_file = open(myconfig.dev_npy_dir + str(step) + '.json', 'wb')
                    pickle.dump(batch, dev_npy_file)
                    dev_npy_file.close()
                elif myconfig.pretrain_mode == 'test' and myconfig.dev_data_file_load == True:
                    dev_npy_file = open(myconfig.dev_npy_dir + str(step) + '.json', 'rb')
                    batch = pickle.load(dev_npy_file)
                    dev_npy_file.close()

                batch = [t.to(device) for t in batch]

                if myconfig.return_the_img_path:
                    input_ids, segment_ids, input_mask, lm_label_ids, masked_pos, masked_weights, is_next, task_idx, img, vis_masked_pos, vis_pe, ans_labels, img_paths = batch
                else:
                ...
                precision.append(torch.sum(lm_label_ids==pretext_loss_deprecated).item()/3/64)
                ...
                for input_id, mask_lm, predict_lm, img_id in zip(input_ids, lm_label_ids, pretext_loss_deprecated,
                                                                 img_paths):
                    cur_data = {}
                    cur_data['input'] = tokenizer.convert_ids_to_tokens(input_id.cpu().tolist())
                    cur_data['mask_lm'] = tokenizer.convert_ids_to_tokens(mask_lm.cpu().tolist())
                    cur_data['predict_lm'] = tokenizer.convert_ids_to_tokens(predict_lm.cpu().tolist())
                    cur_data['img_id'] = img_id.cpu().tolist()[0]
                    if mask_lm.shape[0]!=0:
                        cur_data['score'] = torch.sum(mask_lm == predict_lm).item() / mask_lm.shape[0]
                        cur_data['mask_num'] = mask_lm.shape[0]
                    else:
                        cur_data['score'] = 0
                        cur_data['mask_num'] = 0

                    if myconfig.vis_input == True:
                        cur_data['mode'] = 'multimodal'
                    else:
                        cur_data['mode'] = 'textonly'
                    result_dict.append(cur_data)
            if myconfig.vis_input == True:
                with open(myconfig.result_multimodal_file,'wb') as resf:
                    pickle.dump(result_dict, resf)
                    resf.close()
            else:
                with open(myconfig.result_textonly_file,'wb') as resf:
                    pickle.dump(result_dict, resf)
                    resf.close()

            if args.world_size > 1:
                torch.distributed.barrier()
        score = sum(precision) / len(precision) * 100
        score = round(score, 2)
        print("The Language model:{} precision is: {}% vis_input={}".format(args.model_recover_path.split('/')[-1],
                                                                            score,
                                                                            myconfig.vis_input))
