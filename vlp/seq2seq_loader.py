        ...
        if tasks == 'img2txt':
            ...
            for file_s in file_src:
                ...
                for i in range(1, img_dat.shape[0]):
                    if use_num_imgs == -1 or counter < use_num_imgs:
                        if enable_butd:
                            src_tk = os.path.join(image_root, 'train_val_2014',
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance):
        ...
        masked_pos = cand_pos[2*n_pred:3*n_pred]
        ...
        if not self.enable_butd:
            ...
            if self.region_det_file_prefix != '':
                ...
                # with h5py.File('./data/flickr30k/region_feat_gvd_wo_bgd/flickr30k_detection_vg_thresh0.2_feat_gvd_checkpoint_trainvaltest.h5', 'r') as region_bbox_f:
                #     vis_pe = torch.from_numpy(region_bbox_f[img_id][:])
        ...
        if myconfig.return_the_img_path:
            return (input_ids, segment_ids, input_mask, masked_ids, masked_pos, masked_weights, -1, self.task_idx, img,
                    vis_masked_pos, vis_pe, ans_tk, torch.tensor([img_id]))
        else:
            return (input_ids, segment_ids, input_mask, masked_ids, masked_pos, masked_weights, -1, self.task_idx, img,
